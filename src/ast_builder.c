...
static const ExampleTableOnly* ExampleTableOnly_new(
    const TableRow* table_header, const TableRows* table_body);
...
static wchar_t* create_multi_line_text_from_tokens(int text_length,
                                                   int line_count,
                                                   ItemQueue* line_queue);

static const wchar_t* get_description(AstNode* ast_node);

static const Comments* get_comments(AstBuilder* ast_builder);

static void ensure_cell_count(ErrorList* errors, const TableRows* rows,
                              const TableRow* header, AstNode* ast_node);
...
void AstBuilder_build(Builder* builder, Token* token) {
  if ((RuleType)token->matched_type == Rule_Comment) {
    ...
    AstNode_add(current_node((AstBuilder*)builder),
                (RuleType)token->matched_type, token);
  }
}
...
void AstBuilder_end_rule(Builder* builder, RuleType rule) {
  ...
  AstNode_add(((AstNode*)ItemQueue_peek(((AstBuilder*)builder)->stack)), rule,
              object);
}

const GherkinDocument* AstBuilder_get_result(Builder* builder) {
  return (const GherkinDocument*)AstNode_get_single(
      current_node((AstBuilder*)builder), Rule_GherkinDocument);
}
...
static void* transform_node(AstNode* ast_node, AstBuilder* ast_builder) {
  ...
  switch (ast_node->rule_type) {
    case Rule_Step: {
      ...
      const Step* step =
          Step_new(token->location, token->matched_keyword, token->matched_text,
                   get_step_argument(ast_node));
      Token_delete(token);
    ...
    case Rule_DataTable: {
      ...
      const DataTable* data_table =
          DataTable_new(rows->table_rows[0].location, rows);
      AstNode_delete(ast_node);
      return (void*)data_table;
    }
    case Rule_DocString: {
      token = AstNode_get_token(ast_node, Token_DocStringSeparator);
      const DocString* doc_string = DocString_new(
          token->location, token->matched_text, get_doc_string_text(ast_node));
      Token_delete(token);
      AstNode_delete(ast_node);
    ...
    case Rule_Background: {
      ...
      const Background* background = Background_new(
          token->location, token->matched_keyword, token->matched_text,
          get_description(ast_node), get_steps(ast_node));
      Token_delete(token);
      AstNode_delete(ast_node);
    ...
    case Rule_Scenario_Definition: {
      ...
      if (node) {
        ...
        const Scenario* scenario = Scenario_new(
            token->location, token->matched_keyword, token->matched_text,
            get_description(node), get_tags(ast_node), get_steps(node));
        Token_delete(token);
        AstNode_delete(node);
        AstNode_delete(ast_node);
        ...
        const ScenarioOutline* scenario_outline = ScenarioOutline_new(
            token->location, token->matched_keyword, token->matched_text,
            get_description(node), get_tags(ast_node), get_steps(node),
            get_examples(node));
        Token_delete(token);
    ...
    case Rule_Examples_Definition: {
      ...
      const ExampleTableOnly* table =
          (ExampleTableOnly*)AstNode_get_single(node, Rule_Examples_Table);
      const TableRow* header = table ? table->table_header : 0;
      const TableRows* body = table ? table->table_body : 0;
      const ExampleTable* example_table = ExampleTable_new(
          token->location, token->matched_keyword, token->matched_text,
          get_description(node), get_tags(ast_node), header, body);
    ...
    case Rule_Feature: {
      ...
      const Feature* feature = Feature_new(
          token->location, token->matched_language, token->matched_keyword,
          token->matched_text, get_description(node), get_tags(node),
          get_scenario_definitions(ast_node));
      Token_delete(token);
      AstNode_delete(node);
      AstNode_delete(ast_node);
      return (void*)feature;
    ...
    case Rule_GherkinDocument: {
      ...
      const GherkinDocument* gherkin_document =
          GherkinDocument_new(feature, get_comments(ast_builder));
      AstNode_delete(ast_node);
...
static const ScenarioDefinitions* get_scenario_definitions(AstNode* ast_node) {
  ItemQueue* scenario_definitions_queue =
      AstNode_get_items(ast_node, Rule_Scenario_Definition);
  ...
  ScenarioDefinitions* scenario_definitions =
      (ScenarioDefinitions*)malloc(sizeof(ScenarioDefinitions));
  scenario_definitions->scenario_definition_count =
      background_count + ItemQueue_size(scenario_definitions_queue);
  scenario_definitions->scenario_definitions = 0;
  if (scenario_definitions->scenario_definition_count > 0) {
    scenario_definitions->scenario_definitions = (ScenarioDefinition**)malloc(
        scenario_definitions->scenario_definition_count *
        sizeof(ScenarioDefinition*));
    int i;
    if (background) {
      scenario_definitions->scenario_definitions[0] =
          (ScenarioDefinition*)background;
    }
    for (i = background_count;
         i < scenario_definitions->scenario_definition_count; ++i) {
      scenario_definitions->scenario_definitions[i] =
          (ScenarioDefinition*)ItemQueue_remove(scenario_definitions_queue);
    }
  }
...
static const Examples* get_examples(AstNode* ast_node) {
  ItemQueue* examples_queue =
      AstNode_get_items(ast_node, Rule_Examples_Definition);
  ...
  if (examples->example_count > 0) {
    examples->example_table =
        (ExampleTable*)malloc(examples->example_count * sizeof(ExampleTable));
    int i;
    for (i = 0; i < examples->example_count; ++i) {
      ExampleTable_transfer(&examples->example_table[i],
                            (ExampleTable*)ItemQueue_remove(examples_queue));
    }
  }
  return examples;
}
...
static const TableRow* get_table_header(AstNode* ast_node) {
  ...
  if (!ItemQueue_is_empty(table_rows_queue)) {
    ...
    const TableRow* table_row =
        TableRow_new(token->location, get_table_cells(token));
    Token_delete(token);
    return table_row;
  }
...
static const TableRows* get_table_body(AstNode* ast_node) {
  ...
  if (table_rows->row_count > 0) {
    table_rows->table_rows =
        (TableRow*)malloc(table_rows->row_count * sizeof(TableRow));
    Token* token;
    int i;
    for (i = 0; i < table_rows->row_count; ++i) {
      ...
      TableRow_transfer(
          ...
          (TableRow*)TableRow_new(token->location, get_table_cells(token)));
      Token_delete(token);
    }
  }
  return table_rows;
...
static void ensure_cell_count(ErrorList* errors, const TableRows* rows,
  ...
  int cell_count = header ? header->table_cells->cell_count
                          : rows->table_rows[0].table_cells->cell_count;
  int i;
  for (i = 0; i < rows->row_count; ++i) {
    if (rows->table_rows[i].table_cells->cell_count != cell_count) {
      Location error_location = {rows->table_rows[i].location.line,
                                 rows->table_rows[i].location.column};
...
static const TableCells* get_table_cells(Token* token) {
  ...
  if (table_cells->cell_count > 0) {
    table_cells->table_cells =
        (TableCell*)malloc(table_cells->cell_count * sizeof(TableCell));
    Location location;
    location.line = token->location.line;
    int i;
    for (i = 0; i < table_cells->cell_count; ++i) {
      location.column = token->matched_items->items[i].column;
      TableCell_transfer(&table_cells->table_cells[i],
                         (TableCell*)TableCell_new(
                             location, token->matched_items->items[i].text));
    }
  }
  return table_cells;
}
  ...
  if (tags->tag_count > 0) {
    ...
    while (!ItemQueue_is_empty(tags_queue)) {
      ...
      for (i = 0; i < token->matched_items->count; ++i) {
        location.column = token->matched_items->items[i].column;
        Tag_transfer(&tags->tags[tag_index++], location,
...
static const wchar_t* get_description_text(AstNode* ast_node) {
  ...
  return create_multi_line_text_from_tokens(text_length, line_count,
                                            line_queue);
}
...
static const Comments* get_comments(AstBuilder* ast_builder) {
  ...
  if (comments->comment_count > 0) {
    comments->comments =
        (Comment*)malloc(comments->comment_count * sizeof(Comment));
    int i;
    for (i = 0; i < comments->comment_count; ++i) {
      Token* token = (Token*)ItemQueue_remove(ast_builder->comments);
      Comment_transfer(&comments->comments[i],
                       Comment_new(token->location, token->matched_text));
      Token_delete(token);
    }
  }
  return comments;
